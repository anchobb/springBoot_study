...
public class JwtTokenProvider {
    ...
    public TokenInfo generateToken(Authentication authentication) { //시큐리티 안에 있는 사용자정보중에
        // 권한 가져오기
        String authorities = authentication.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)//role을 받아서 권한을 받아옴.
                .collect(Collectors.joining(","));
        long now = (new Date()).getTime(); //시간 받아옴
        ...
        String accessToken = Jwts.builder()
                ...
                .claim("auth", authorities)//역할정보저장
                .setExpiration(accessTokenExpiresIn) //엑세스토큰의 만료날짜 정하기(기한이 짧을수록 안정적)
        ...
        return TokenInfo.builder() //TokenInfo클래스에 전달
                .grantType("Bearer")
