...
//        System.out.println("FILES's LENGTH : " +files.length);
    ...
    public boolean removeFile(Long no, String filename) {
        ...
        filename = filename.trim(); //filename에 공백이 있을수도 있으니까 trim으로 제거
...
//        System.out.println("FILES's SIZE : "  + files[0].getSize());
        ...
        if(dto.getFiles().length >= 1 && dto.getFiles()[0].getSize()!=0L) //받은 dto에 파일정보가 들어가 있으면!
        {
            //Upload Dir 미존재시 생성
